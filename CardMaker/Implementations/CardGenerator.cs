...
namespace CardMaker.Implementations
{
    public class CardGenerator : IMakeCards {
        public void MakeCards() {
            var deckDefinition = JsonConvert.DeserializeObject<Deck>(File.ReadAllText(@"C:\temp\testdeck.cm"));

            var deck = CreateCards(deckDefinition);
            //PublishCards(deck);

        }

        private List<MagickImage> CreateCards(Deck deck) {
        ...
        private void PublishCards(List<MagickImage> deck) {
